...
import os
from utils.errors import NotGithubDomain
...
class GithubLoader:
    ...
    def _parse_repo(self, url: str):
        ...
        if parsed_url.netloc != "github.com":
            raise NotGithubDomain("invalid domain")
        
        return  parsed_url.path.split("/tree")[0][1:]
    
    def load(self):
        self.loader = GithubFileLoader(access_token=os.getenv("GITHUB_TOKEN"),     
                        github_api_url="https://api.github.com",
                        file_filter=lambda file_path: self.path in file_path,
                        branch="main",repo=self.repo,
                        )
